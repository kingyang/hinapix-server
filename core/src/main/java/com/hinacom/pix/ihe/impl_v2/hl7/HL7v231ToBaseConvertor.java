...
public class HL7v231ToBaseConvertor {
    ...
    public PatientIdentifier getAccountNumber() {
        PatientIdentifier accountNumber = new PatientIdentifier();
        ...
        //Need to reconcile assigning authority to fill up any missing components
        Identifier reconciledAssignAuth = AssigningAuthorityUtil.reconcileIdentifier(assignAuth, connection);
        accountNumber.setAssigningAuthority(reconciledAssignAuth);
        accountNumber.setAssigningFacility(assignFac);
        accountNumber.setId(cx.getID().getValue());
        accountNumber.setIdentifierTypeCode(cx.getIdentifierTypeCode().getValue());
        // identifier.setEffectiveDate(cx.get);
        //	identifier.setExpirationDate(expirationDate);
